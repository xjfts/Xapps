...
{
	int err, control;
	int temp, oldpage, mscr;

	//set fiber register page 1
	err = phy_write(phydev, MII_MARVELL_PHY_PAGE, 
					MII_M1111_FIBER);
	if (err < 0)
		return err;
	...
	err = phy_write(phydev, 20, (0x40 | 0x03));
	if (err < 0)
		return err;
	...
	if (phydev->speed == 100) {
			printk(KERN_INFO "m88e1510_config_init control: %d\n", control);
			control |= IEEE_CTRL_LINKSPEED_100M;
			/* Dont advertise PHY speed of 1000 Mbps */
			temp = phy_read(phydev, MII_ADVERTISE);
			temp &= ~0x60;
			/* Dont advertise PHY speed of 10 Mbps */
			err = phy_write(phydev, MII_ADVERTISE, temp);
			if (err < 0)
				return err;
	}
	//////////////////////////////////////////////
	err = phy_write(phydev, MII_BMCR, control | BMCR_RESET);
	if (err < 0)
		return err;
	...
	//////////////////////////////////////////
	printk(KERN_INFO "m88e1510_config_init exit\n");
